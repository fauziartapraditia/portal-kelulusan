...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name !== '' && $exam_number !== '' && $nisn !== '') {
        ...
        $stmt = $pdo->prepare("SELECT id FROM students WHERE exam_number = :ex OR nisn = :nisn");
        $stmt->execute(['ex' => $exam_number, 'nisn' => $nisn]);
        if ($stmt->fetch()) {
            $error = "Nomor Ujian atau NISN tersebut sudah terdaftar dalam sistem.";
        } else {
            // Save student
            $stmt = $pdo->prepare("INSERT INTO students (exam_number, nisn, name, major, class_name, status, notes, subject_scores, prank_level) 
                                   VALUES (:exam_number, :nisn, :name, :major, :class_name, :status, :notes, :subject_scores, :prank_level)");
            $stmt->execute([
...
?>
    ...
    <main class="main-content container" style="max-width: 800px; width:100%; padding: 48px 24px;">
        ...
        <div class="mb-6">
            <h2 style="font-size: 1.75rem; font-weight: 800; letter-spacing: -0.025em; margin-bottom: 4px;">Tambah Siswa Baru</h2>
            <p class="text-secondary">Masukkan biodata dan status kelulusan siswa secara lengkap.</p>
        </div>
        ...
        <div class="card">
            ...
            <form action="/kelulusan/admin/siswa/tambah" method="POST">
                
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px;">
                    <div class="form-group">
                        <label for="name" class="form-label">Nama Lengkap Siswa <span style="color: var(--error);">*</span></label>
                        <input type="text" id="name" name="name" class="form-control" placeholder="Contoh: Muhammad Yusuf" required autofocus>
                    </div>

                    <div class="form-group">
                        <label for="exam_number" class="form-label">Nomor Peserta Ujian <span style="color: var(--error);">*</span></label>
                        <input type="text" id="exam_number" name="exam_number" class="form-control" placeholder="Contoh: 12-345-678-9" required>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px;">
                    <div class="form-group">
                        ...
                        <input type="text" id="nisn" name="nisn" class="form-control" placeholder="Contoh: 0041234567" required>
                    </div>

                    <div class="form-group">
                        <label for="major" class="form-label">Kompetensi Keahlian (Jurusan) <span style="color: var(--error);">*</span></label>
                        <input type="text" id="major" name="major" class="form-control" placeholder="Contoh: Teknik Komputer dan Jaringan" required>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px;">
                    <div class="form-group">
                        ...
                        <input type="text" id="class_name" name="class_name" class="form-control" placeholder="Contoh: XII TKJ 1" required>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <select id="prank_level" name="prank_level" class="form-control" style="height: 50px;">
                        ...
                        <option value="1_MENIT">Prank 1 Menit - Notif Normal</option>
                        <option value="30_MENIT">Prank 30 Menit - Notif Sedang</option>
                        <option value="2_JAM">Prank 2 Jam - Notif Tinggi</option>
                        <option value="3_JAM">Prank 3 Jam - Notif Sangat Tinggi & Menakutkan</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="notes" class="form-label">Catatan Tambahan (Opsional)</label>
                    <textarea id="notes" name="notes" class="form-control" placeholder="Contoh: Hubungi pihak sekolah untuk penyerahan SKL" style="height: 100px; resize: vertical;"></textarea>
                </div>

                <div style="display: flex; gap: 12px; margin-top: 32px;">
                    <button type="submit" class="btn btn-primary" style="flex: 1;">Simpan Siswa</button>
